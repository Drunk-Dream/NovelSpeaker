...
public sealed class StyleGallerySceneTests
{
    ...
    private void Scene_registry_groups_concrete_gallery_scenes_with_fixed_dimensions()
    {
        ...
        Assert.Equal(
            ["button-styles", "feedback", "form-field", "input-controls", "interaction-states", "list-components", "media-controls", "menus", "navigation", "page-header", "palette-probe", "progress", "provider-controls", "provider-style-probe", "rules-shared", "section-surface", "selection", "settings-controls", "status-view", "surfaces", "theme-resource-probe", "token-components", "typography"],
        ...
        Assert.Equal(
            ["provider-style-probe", "theme-resource-probe", "palette-probe", "interaction-states", "token-components", "typography", "surfaces"],
    ...
    private void Gallery_window_scene_selector_exposes_the_three_scene_groups()
    {
        WpfTestHost.RunInSta(() =>
        {
            ...
            Assert.Equal(23, view.Cast<GallerySceneDefinition>().Count());
