...
- SpeechText 为空，或只包含空白、标点、分隔符和装饰符号而没有 Unicode 字母/数字：该段不请求 TTS，并由播放状态机安全跳过；例如小说中常见的 `………` 分隔段仍可显示，但不计入朗读清单、主动缓存或缓存完整度分母。
...
- 左侧卡片采用左右布局：左侧名称、Pattern 摘要，右侧 ToggleSwitch。启用状态即时持久化，不属于编辑草稿或 Dirty State。
- 不显示拖动手柄或“更多”按钮。右键菜单提供“导出到文件”“复制到剪切板”、上移、下移、删除，且右键不切换当前编辑对象。
- 排序以整卡长按约 `300 ms` 后拖动为主；拖动态使用轻量反馈和插入线，支持列表边缘自动滚动，不实现相邻卡片位移动画，Drop 后才持久化顺序。
- 右侧：名称、Pattern、Replacement、作用目标、帮助、取消、保存。
- 页面进入时不自动选中规则；单击卡片后才打开编辑器。编辑器打开后“取消”始终可用并直接丢弃草稿、关闭编辑器；“保存”只在草稿修改且校验通过时可用。
- 新建规则先形成编辑副本，保存后才进入正式列表。
- 切换规则、返回或退出时复用统一 dirty-state 导航保护。
- 从文件/剪切板导入采用合并策略：完全重复跳过，同名不同内容作为新规则并按源顺序追加到现有排序末尾；导入后不自动打开规则。
...
- 编辑 dirty state、显式取消关闭编辑器、启用状态与草稿解耦、排序和导航保护。
- 文件/剪切板合并导入、重复跳过、同名不同内容新增、单规则文件/剪切板导出。
- 长按拖动阈值、插入位置、边缘自动滚动、Drop 后持久化和上移/下移备用排序。
