...
- 普通目录只补建已有但过期的计划，不为从未建立计划的普通章节创建计划。
- 缓存管理页只对当前查询到的有缓存章节补建缺失或过期计划。
- 播放、预取、主动缓存和导出继续在消费前确保对应计划为当前版本。
- 有限并发、可取消并登记到后台任务 registry。
- 同一书籍/章节的并发请求合并为一个共享任务；首轮 UI 查询立即返回“更新中/空白”，不等待正文处理完成。
...
缓存健康维护由进程启动流程负责，负责检查缺失或损坏文件并修正索引。启动维护必须在 Shell 可交互、播放、预取或主动缓存任务启动前完成；这样计划提交到首个缓存索引写入之间不会发生孤立计划清理竞态。它不得在目录或缓存管理页查询期间同步运行，也不得与当前播放争用长时间解码资源。
