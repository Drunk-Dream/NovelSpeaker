...
章节朗读清单由播放、预取、主动缓存和导出等真正消费章节内容的用例按需建立或更新；完整度读取发现过期计划时由进程级缓存工作区 owner 异步补建，缓存管理页对有缓存但缺失计划的章节同样补建，同章请求合并为一个后台任务。普通目录遇到缺失计划不建立清单；所有页面首轮查询只聚合 SQLite 计划和 `Ready` 索引，不重新处理正文、不逐文件解码。正式缓存写入前必须先提交对应计划。删除某章最后一条缓存索引时同步回收其朗读清单。朗读清单允许在单次运行中暂时先于缓存存在，但启动缓存维护在完成缺失/损坏索引修复和容量淘汰后，还必须集合式回收所有不存在任何 `AudioCacheEntries` 的 `ChapterSpeechPlans`；段表依靠外键级联删除。该维护只收敛持久化残留，不在播放/生成中的“计划已提交、缓存尚未落盘”窗口内做即时删除。
...
- 正式应用数据根目录为程序目录下的 `Data/`；开发启动使用独立数据根，自动测试使用测试自己拥有的临时根。
- 本次从旧 `%LocalAppData%/NovelSpeaker` 切换到便携数据目录不建立发现、迁移、回退或双读兼容路径；SQLite schema migration 体系仍独立保留。
