...
当用户要求“更新版本号并发布”时，必须依次完成：

1. 更新项目版本号，包括包、程序集、文件和信息版本。
2. 提交本次发布包含的代码和版本变更。
3. 在发布提交上创建符合 `vX.Y.Z` 格式的 Git 标签。
4. 推送目标分支和该标签，以触发发布工作流。
5. 等待发布工作流成功，确认 GitHub Release 以及 ZIP、SHA256 资产可用。
6. 发布成功后更新该 GitHub Release 的说明，以面向用户的结构化内容概括主要变更、缺陷修复和必要的验证或下载信息，并再次核对说明和资产；在此步骤完成前不得宣告发布完成。
