...
Release workflow 创建的自动 notes 只是初始内容。最终 Release Note 必须使用中文（专有名词、命令和文件名可保留英文），根据真实发布范围重新整理。

先按正式 Release 的版本顺序确认紧邻的上一版 tag，再依据：
...
- 以两个相邻正式发布 tag 的最终净差异为范围；合并、重复提交或曾经进入分支但未留在最终版本的改动不能重复归属，不能把早期累积功能或下一版改动写成本版新增；
- 首个版本没有上一版 tag，应按该 tag 的实际产品能力编写首次发布说明，不虚构版本对比；
- 面向用户描述实际变化；
...
- 问题修复描述用户可见问题与影响场景，不无证据推断根因；
- 保留 `<previous_tag>...<release_tag>` 的完整 compare link；首版改用该 tag 的源码链接。

统一使用中文标题，按真实内容选择分组，没有内容的分组省略：
...
- `迁移与使用影响`
- `测试与质量`
- `使用提示`：仅在有实际使用、兼容或安全提示时加入；首版可说明首次运行方式和已知限制。
- `下载`：列出该版本实际存在的 ZIP 和 SHA-256 校验文件。

正文末尾统一写“完整变更”对比链接；首版写“版本源码”链接。发布前核对当前 Release 正文和资产，保留仍然准确的用户须知；发布后逐版回读正文，确认中文格式、版本归属、链接和资产名称一致。
