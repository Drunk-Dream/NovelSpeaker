...
From the repository root, generate both themes and the task 7 button scene with:
...
The command exits after writing `manifest.json` and one PNG per selected scene/theme. The task must be selected explicitly (`03` through `07`); omitting `--task` preserves the existing task `03` default and output directory. Use `--scene button-styles` to render only the task 7 button fixture. The manifest records the selected task, scene, theme, fixed size, 96 DPI and SHA-256 for every PNG.
