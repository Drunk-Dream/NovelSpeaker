...
From the repository root, generate both themes and a task 7 or task 8 scene with:
...
The command exits after writing `manifest.json` and one PNG per selected scene/theme. The task must be selected explicitly (`03` through `08`); omitting `--task` preserves the existing task `03` default and output directory. Use `--scene button-styles` for the task 7 button fixture or `--scene media-controls` for the task 8 media fixture. The media fixture includes equal play/pause geometry, template glyph color projection, an Accent played track, a neutral unplayed track, and a volume button without production commands. The slider tooltip displays a deterministic `x / y` projection, and changing its value updates only the Gallery fixture. The manifest records the selected task, scene, theme, fixed size, 96 DPI and SHA-256 for every PNG.
