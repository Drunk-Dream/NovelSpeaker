...
3. 打开书籍开始播放；播放页支持章节/段落切换、语速调整、播放音量、播放进度和正文自动定位。播放音量只影响应用内音频，不改变系统音量，并会在重启后恢复。可在“设置 → 播放设置”开启朗读标题。播放页的“缓存”可多选章节并启动后台主动缓存，“定时停止”和“迷你播放器”也从这里打开。
...
- 播放页和迷你播放器提供共享且可持久化的应用内播放音量滑块，不调整系统音量。
...
详细设计与当前开发计划见 [docs/README.md](docs/README.md)。
